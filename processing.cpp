...
void compute_energy_matrix(const Image* img, Matrix* energy) {
  ...
  if (Image_height(img) < 2 || Image_height(img) < 2) {
    return;
  }
  // Computing energy
  for (int i = 1; i < Image_height(img) - 1; i++) {
    for (int j = 1; j < Image_width(img) - 1; j++) {
      Pixel northPixel = Image_get_pixel(img, (i + 1), j);
      Pixel southPixel = Image_get_pixel(img, (i - 1), j);
      Pixel eastPixel = Image_get_pixel(img, i, (j + 1));
      Pixel westPixel = Image_get_pixel(img, i, (j - 1));
      computedEnergy = squared_difference(northPixel, southPixel) + squared_difference(westPixel, eastPixel);
      // Update max energy
      if (computedEnergy > maxEnergy) {
        maxEnergy = computedEnergy;
      }
      *Matrix_at(energy, i, j) = computedEnergy;
    }
  }
  // Fill borders of energy with max energy
...
void find_minimal_vertical_seam(const Matrix* cost, int seam[]) {
  int colNum;
  int temp;
  // Find col number of bottom row
  colNum = Matrix_column_of_min_value_in_row(cost, Matrix_height(cost) - 1, 0, Matrix_width(cost));
  // Set last element of seam array to colNum
  seam[Matrix_height(cost) - 1] = colNum;
  temp = colNum;
  // Find minimum cost pixel above
  for (int i = Matrix_height(cost) - 2; i >= 0; i--) {
    // Left side
    if (temp == 0) {
      colNum = Matrix_column_of_min_value_in_row(cost, i, 0, 2);
    }
    // Right side
    else if (temp == Matrix_width(cost) - 1) {
      colNum = Matrix_column_of_min_value_in_row(cost, i, temp - 1, temp + 1);
    }
    else {
      colNum = Matrix_column_of_min_value_in_row(cost, i, (temp - 1), (temp + 2));
    }
    seam[i] = colNum;
    temp = colNum;
  }
